...
# ---------------------------------------------------------------------------
# Subcommand: generate — repositories.nix
# ---------------------------------------------------------------------------
...
def _render_nix(repositories: dict[str, Repository]) -> str:
    ...
    for name, repo in sorted(repositories.items()):
        deps = " ".join(json.dumps(d) for d in repo.directDeps)
        libs = " ".join(json.dumps(l) for l in repo.systemLibraries)
        lines += [
            f"  {json.dumps(name)} = {{",
            f"    pin = {json.dumps(repo.pin)};",
            f"    snapshotVersion = {json.dumps(repo.snapshotVersion)};",
            f"    minimumZigVersion = {json.dumps(repo.minimumZigVersion)};",
            f"    directDeps = [ {deps} ];",
...
def cmd_generate(args: argparse.Namespace) -> int:
    ...
    print(f"inspected {len(repositories)} Zig repositories")
    return 0
    ...
    for zh in sorted(sources):
        ...
        if src.kind == "git":
            ...
            lines += [
                ...
                f"      rev = {json.dumps(src.rev)};",
                "      fetchSubmodules = false;",
                f"      hash = {json.dumps(src.nix_hash)};",
                "    };",
            ]
        else:
            lines += [
...
def _render_lock(hashes: Iterable[str]) -> str:
    ...
    lines.extend(f"  {json.dumps(h)}" for h in sorted(hashes))
    lines.append("]")
    return "\n".join(lines) + "\n"
...
def _check_framework() -> None:
    ...
    required_files = [
        ...
        "README.md", "MAINTENANCE.md", "VALIDATION.md",
